...
# ------------------------------------------------------------------------------------------------------------------------------------------------------
# ITER   | EP  | HOLES | DISTANCE   | DIJK_DIST  | DIFF_%   | DEST_OK | STP/30 | LAST_R     | RETURN     | MIN_R    | MAX_R    | MEAN_R   | STD_R   
# ------------------------------------------------------------------------------------------------------------------------------------------------------
...
header = (
    ...
    f"{n_step_header:<7} | {'LAST_R':<10} | {'RETURN':<10} | {'MIN_R':<8} | "
    ...
    for idx, row in df.iterrows(): # Itera sulle righe del csv, ogni riga è un'iterazione (un'insieme di episodi)
        ...
        for ep_num, length in enumerate(lengths):
            ...
            try:
                ...
                ep_hole = ep_dist = ep_dijkstra = ep_dest = last_step_reward = np.nan
                ep_diff_str = f"{'-':^8}"
