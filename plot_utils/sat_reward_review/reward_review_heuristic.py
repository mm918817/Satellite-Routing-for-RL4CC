...
class RewardValidator:
    def __init__(self, topo_file, eval_file, dijkstra_file, w_step=1.0, w_dest=1.0):
        self.w_step = w_step
        self.w_dest = w_dest
        self.MAX_HOPS = 9 # 30 Dal cap massimo per l'ambiente come numero max di step per episodio
    ...
    def run_validation(self, output_path="reward_report"):
        ...
        for flow in self.eval_flows:
            ...
            # Cerca il percorso finché non raggiunge la fine o supera il cap massimo
            while curr_id != e_id and len(path) - 1 < self.MAX_HOPS:
                s_curr = current_topo[curr_id]
                neighbors_dict = s_curr["neighbors"]
                neighbor_ids = [int(v) for v in neighbors_dict.values() if str(v) != "None"] # Considera solo vicini validi
                
                best_neighbor = None
                min_dist = float('inf')
                
                # Trova il vicino che minimizza la distanza dalla destinazione
                for nid in neighbor_ids:
                    d = self.get_dist(current_topo[nid], end_sat)
                    #print(f" Sat {nid}, dista {d} km dalla dest:{end_sat["id"]}") #DEBUG
                    if d < min_dist:
                        min_dist = d
                        best_neighbor = nid
